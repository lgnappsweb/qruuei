...
const PreviewDialog = ({ data, onClose, onSave, formTitle }: { data: any | null; onClose: () => void; onSave: (data: any) => void; formTitle: string; }) => {
  ...
  const handleShare = () => {
    ...
    const report = data;
    const fieldOrder = ['rodovia', 'ocorrencia', 'qth', 'sentido', 'localArea', '---VEHICLES---', 'vtrApoio', 'vtrApoioDescricao', 'danoPatrimonio', 'danoPatrimonioDescricao', 'observacoes', 'auxilios'];
...
export default function OcorrenciaTO39Page() {
  ...
  React.useEffect(() => {
    try {
        ...
        if (editDataString) {
            ...
            if(editData.formPath === '/ocorrencias/to39') {
                const reportToLoad = editData.fullReport;
                
                const arrayFields = ['vehicles'];
  ...
  function handleSave(data: any) {
    try {
        ...
        const formTitle = "DESTOMBAMENTO DE VEÍCULO (TO39)";
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto pb-24">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea
                          placeholder="Descreva os auxílios prestados. Ex: PR32, PR13"
                          className="resize-none"
                          {...field}
                        />
                      </FormControl>
      ...
      <PreviewDialog data={previewData} onClose={() => setPreviewData(null)} onSave={handleSave} formTitle="DESTOMBAMENTO DE VEÍCULO (TO39)" />
    </div>
  );
}
