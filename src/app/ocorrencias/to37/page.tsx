...
const PreviewDialog = ({ data, onClose, onSave, formTitle }: { data: any | null; onClose: () => void; onSave: (data: any) => void; formTitle: string; }) => {
  ...
  const handleShare = () => {
    ...
    const sections: { title: string, fields: (keyof z.infer<typeof formSchema>)[] }[] = [
        ...
        { title: 'Sinalização', fields: ['tipoPlaca', 'acao', 'nomePlaca', 'quantidade'] },
...
export default function OcorrenciaTO37Page() {
  ...
  React.useEffect(() => {
    try {
        ...
        if (editDataString) {
            ...
            if(editData.formPath === '/ocorrencias/to37') {
  ...
  async function handleSave(data: any) {
    ...
    try {
        const formTitle = "SINALIZAÇÃO VERTICAL (TO37)";
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto pb-24">
      ...
      <PreviewDialog data={previewData} onClose={() => setPreviewData(null)} onSave={handleSave} formTitle="SINALIZAÇÃO VERTICAL (TO37)" />
    </div>
  );
}
