...
const formSchema = z.object({
  ...
  tipoPropaganda: z.string().optional(),
...
const PreviewDialog = ({ data, onClose, onSave, formTitle }: { data: any | null; onClose: () => void; onSave: (data: any) => void; formTitle: string; }) => {
  ...
  const handleShare = () => {
    ...
    const sections: { title: string, fields: (keyof z.infer<typeof formSchema>)[] }[] = [
        { title: 'Informações Gerais', fields: ['rodovia', 'ocorrencia', 'qth', 'sentido', 'localArea'] },
        { title: 'Detalhes da Propaganda', fields: ['tipoPropaganda', 'tipoPropagandaOutros', 'acao', 'mensagem', 'quantidade'] },
    ...
    sections.forEach(section => {
        ...
        section.fields.forEach(key => {
            ...
            if (value !== 'NILL' && value !== '' && (!Array.isArray(value) || value.length > 0)) {
                if ((key === 'tipoPropagandaOutros' && data.tipoPropaganda !== 'Outros') || (key === 'vtrApoioDescricao' && !data.vtrApoio)) return;
...
export default function OcorrenciaTO38Page() {
  ...
  React.useEffect(() => {
    try {
        ...
        if (editDataString) {
            ...
            if(editData.formPath === '/ocorrencias/to38') {
  ...
  function handleSave(data: any) {
    try {
        ...
        const formTitle = "PLACAS DE PROPAGANDA (TO38)";
