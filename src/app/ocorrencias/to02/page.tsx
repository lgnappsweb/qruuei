...
const PreviewDialog = ({ data, onClose, onSave, formTitle }: { data: any | null; onClose: () => void; onSave: (data: any) => void; formTitle: string; }) => {
  ...
  const handleShare = () => {
    ...
    const fieldsOrder = ['rodovia', 'ocorrencia', 'qth', 'sentido', 'localArea', 'qthInicio', 'qthTermino', 'proporcaoMetros', 'areaTotal', 'vtrApoio', 'vtrApoioDescricao', 'observacoes', 'auxilios'];
    ...
    fieldsOrder.forEach(key => {
        ...
        if (value !== 'NILL' && value !== '' && (!Array.isArray(value) || value.length > 0)) {
            ...
            text += `*${formatLabel(key).toUpperCase()}:* ${renderSimpleValue(value)}\n`;
        }
    });
            ...
            <div className="space-y-6">
                <Card>
                    ...
                    <CardContent className="text-xl space-y-4 pt-6">
                        ...
                        <Field label="proporcaoMetros" value={data.proporcaoMetros} />
                        <Field label="areaTotal" value={data.areaTotal} />
                    </CardContent>
                </Card>
                <Card>
...
export default function OcorrenciaTO02Page() {
  ...
  React.useEffect(() => {
    try {
        ...
        if (editDataString) {
            ...
            if(editData.formPath === '/ocorrencias/to02') {
  ...
  function handleSave(data: any) {
     try {
        ...
        const formTitle = "INCÊNDIO NA FAIXA DE DOMÍNIO / LINDEIRO (TO02)";
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto pb-24">
      ...
      <PreviewDialog data={previewData} onClose={() => setPreviewData(null)} onSave={handleSave} formTitle="INCÊNDIO NA FAIXA DE DOMÍNIO / LINDEIRO (TO02)" />
    </div>
  );
}
