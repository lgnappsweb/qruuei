...
const formSchema = z.object({
  ...
  // Detalhes do Dano
  tipoItemDanificado: z.string().optional(),
  tipoItemDanificadoOutros: z.string().optional(),
  descricaoDano: z.string().optional(),
  causaProvavel: z.string().optional(),
  causaProvavelOutros: z.string().optional(),
  extensaoDano: z.string().optional(),
...
const PreviewDialog = ({ data, onClose, onSave, formTitle }: { data: any | null; onClose: () => void; onSave: (data: any) => void; formTitle: string; }) => {
  ...
  const handleShare = () => {
    ...
    const sections: { title: string, fields: (keyof z.infer<typeof formSchema>)[] }[] = [
        ...
        { title: 'Detalhes do Dano', fields: ['tipoItemDanificado', 'tipoItemDanificadoOutros', 'descricaoDano', 'causaProvavel', 'causaProvavelOutros', 'extensaoDano', 'nivelRisco'] },
    ...
    sections.forEach(section => {
        ...
        section.fields.forEach(key => {
            ...
            if (value !== 'NILL' && value !== '' && (!Array.isArray(value) || value.length > 0)) {
                if ((key === 'tipoItemDanificadoOutros' && data.tipoItemDanificado !== 'Outro') ||
                    (key === 'causaProvavelOutros' && data.causaProvavel !== 'Outra') ||
      ...
      <DialogContent className="max-w-4xl h-[90vh] flex flex-col">
        ...
        <ScrollArea className="flex-1 pr-6 -mr-6 mt-4">
            <div className="space-y-6">
                ...
                <Card className="shadow-xl hover:shadow-2xl shadow-black/20 dark:shadow-lg dark:hover:shadow-xl dark:shadow-white/10">
                    <CardHeader><CardTitle>Detalhes do Dano</CardTitle></CardHeader>
                    <CardContent className="text-xl space-y-4 pt-6">
                        <Field label="tipoItemDanificado" value={data.tipoItemDanificado} />
                        {data.tipoItemDanificado === 'Outro' && <Field label="tipoItemDanificadoOutros" value={data.tipoItemDanificadoOutros} />}
                        <Field label="descricaoDano" value={data.descricaoDano} />
                        <Field label="causaProvavel" value={data.causaProvavel} />
                        {data.causaProvavel === 'Outra' && <Field label="causaProvavelOutros" value={data.causaProvavelOutros} />}
                        <Field label="extensaoDano" value={data.extensaoDano} />
                        <Field label="nivelRisco" value={data.nivelRisco} />
                    </CardContent>
                </Card>
                <Card className="shadow-xl hover:shadow-2xl shadow-black/20 dark:shadow-lg dark:hover:shadow-xl dark:shadow-white/10">
...
export default function DanosEstruturaisPage() {
  ...
  React.useEffect(() => {
    try {
        ...
        if (editDataString) {
            ...
            if(editData.formPath === '/ocorrencias/to15/danos-estruturais') {
  ...
  function handleSave(data: any) {
    try {
        ...
        const formTitle = "DANOS ESTRUTURAIS NA VIA (TO15)";
        ...
        const ocorrenciaData = {
            ...
            formPath: '/ocorrencias/to15/danos-estruturais'
        };
    ...
    <div className="space-y-8 max-w-4xl mx-auto pb-24">
      ...
      <div className="space-y-2 text-center">
        ...
        <p className="text-muted-foreground">
          Preencha os campos para registrar danos em meio-fio, canaletas, etc.
        </p>
      </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <Card className="shadow-xl hover:shadow-2xl shadow-black/20 dark:shadow-lg dark:hover:shadow-xl dark:shadow-white/10">
            ...
            <CardContent className="space-y-6 pt-6">
                <FormField
                  control={form.control}
                  name="tipoItemDanificado"
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Tipo de Item Danificado</FormLabel>
                       <Select onValueChange={field.onChange} value={field.value}>
                        <FormControl>
                          <SelectTrigger>
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="Canaleta/Drenagem">Canaleta/Drenagem</SelectItem>
                          <SelectItem value="Sarjeta">Sarjeta</SelectItem>
                          <SelectItem value="Defensa metálica">Defensa metálica</SelectItem>
                          <SelectItem value="Barreira de concreto">Barreira de concreto</SelectItem>
                          <SelectItem value="Tachas/Tachões">Tachas/Tachões</SelectItem>
                          <SelectItem value="Cerca de limite">Cerca de limite</SelectItem>
                          <SelectItem value="Ponte/Viaduto (estrutura)">Ponte/Viaduto (estrutura)</SelectItem>
                ...
                />
                {form.watch('tipoItemDanificado') === 'Outro' &&
                    <FormField control={form.control} name="tipoItemDanificadoOutros" render={({ field }) => (<FormItem><FormLabel>Especifique o item</FormLabel><FormControl><Input placeholder="Descreva o item danificado" {...field} /></FormControl><FormMessage /></FormItem>)}/>
                }
                 <FormField
                    control={form.control}
                    name="descricaoDano"
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Descrição Detalhada do Dano</FormLabel>
                        <FormControl>
                          <Textarea placeholder="Ex: Canaleta quebrada, meio-fio trincado..." {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                 ...
                 <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Causa Provável do Dano</FormLabel>
                       <Select onValueChange={field.onChange} value={field.value}>
                        <FormControl>
                          <SelectTrigger>
                        ...
                        <SelectContent>
                          <SelectItem value="Colisão de veículo">Colisão de veículo</SelectItem>
                          <SelectItem value="Vandalismo">Vandalismo</SelectItem>
                          <SelectItem value="Desgaste natural">Desgaste natural</SelectItem>
                          <SelectItem value="Ação climática">Ação climática (chuva, vento)</SelectItem>
                          <SelectItem value="Ação de terceiros">Ação de terceiros (obra, etc)</SelectItem>
                ...
                />
                 {form.watch('causaProvavel') === 'Outra' &&
                    <FormField control={form.control} name="causaProvavelOutros" render={({ field }) => (<FormItem><FormLabel>Especifique a causa</FormLabel><FormControl><Input placeholder="Descreva a causa provável" {...field} /></FormControl><FormMessage /></FormItem>)}/>
                }
                <FormField
                    control={form.control}
                    name="extensaoDano"
                    render={({ field }) => (
                    <FormItem>
                        <FormLabel>Extensão Aproximada do Dano</FormLabel>
                        <FormControl>
                        <Input placeholder="Ex: 5 metros, 2 peças" {...field} />
                        </FormControl>
                        <FormMessage />
                    </FormItem>
                    )}
                    ...
                    <FormItem>
                      <FormLabel>Nível de Risco</FormLabel>
                       <Select onValueChange={field.onChange} value={field.value}>
                        ...
                        <SelectContent>
                          <SelectItem value="Alto (risco iminente à segurança)">Alto (risco iminente à segurança)</SelectItem>
                          <SelectItem value="Médio (requer atenção)">Médio (requer atenção)</SelectItem>
                          <SelectItem value="Baixo (dano menor/cosmético)">Baixo (dano menor/cosmético)</SelectItem>
                        </SelectContent>
                      </Select>
                      <FormMessage />
                    </FormItem>
      ...
      <PreviewDialog data={previewData} onClose={() => setPreviewData(null)} onSave={handleSave} formTitle="DANOS ESTRUTURAIS NA VIA (TO15)" />
    </div>
  );
}
